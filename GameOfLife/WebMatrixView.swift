        ...
        if let matrix = matrix {
                ...
                for column in 0..<matrix.width {
                    if matrix[row, column] == true {
                        ...
                        if mode == .Edit {
                            ...
                            let text = CFAttributedStringCreate(nil, "\(matrix.numberOfNeighbours(column, row: row))", attr)
            ...
            if mode == .Edit {
                ...
                for row in 0..<matrix.height {
                    ...
                    CGContextAddLineToPoint(context, CGFloat(matrix.width) * cellSize, CGFloat(row) * cellSize)
                    CGContextStrokePath(context)
                }
                
                for column in 0..<matrix.width {
                    CGContextMoveToPoint(context, CGFloat(column) * cellSize, 0)
                    CGContextAddLineToPoint(context, CGFloat(column) * cellSize, CGFloat(matrix.height) * cellSize)
                    CGContextStrokePath(context)
                }
...
private extension GameOfLifeMatrix {
    func neighbours(row: Int, column: Int) -> [(row: Int, column: Int)] {
        ...
        if column < width - 1 &&  self[row, column + 1] { neighbours.append((row, column + 1)) }
        if row > 0 &&               self[row - 1, column] { neighbours.append((row - 1, column)) }
        if row < height - 1 &&        self[row + 1, column] { neighbours.append((row + 1, column)) }
        
        if column > 0 && row > 0 &&                   self[row - 1, column - 1] { neighbours.append((row - 1, column - 1)) }
        if column < width - 1 && row > 0 &&         self[row - 1, column + 1] { neighbours.append((row - 1, column + 1)) }
        if column < width - 1 && row < height - 1 &&  self[row + 1, column + 1] { neighbours.append((row + 1, column + 1)) }
        if column > 0 && row < height - 1 &&            self[row + 1, column - 1] { neighbours.append((row + 1, column - 1)) }
